...
subcategory: "요리" # 하위 주제 하나. 예: 취미=요리/그림/화분, 스터디=C++/디자인 패턴/영어/TypeScript/Node.js/알고리즘
---
```

- `subcategory`는 카테고리 안의 세부 주제를 나타내는 자유 문자열이야. 기존에 없던
  새로운 주제여도 괜찮음(예: "베이킹", "Rust") — 폴더나 라우트를 새로 만들 필요 없이
  프론트매터에 값만 넣으면 `/hobby/topic/{주제}/` 또는 `/study/topic/{주제}/` 페이지가
  자동으로 생성됨. 같은 주제는 항상 똑같은 문자열로 통일할 것(예: "TypeScript"를 어떤
  글은 "타입스크립트"로 쓰면 다른 주제로 분리되어버림).
